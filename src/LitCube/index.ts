...
const msaaSamples = 4;
const vec4SizeBytes = 4 * 4; // vector of 4 floats
...
export default class LitCube extends WebGpuRenderer {
    ...
    async #setupResources() {
        ...
        const colourTex = await this.#createTextureFromImageUrl(`${assetsBaseUrl}/Brick_Wall_019_basecolor.jpg`, "brick colour texture");
        this.#cubeTextureView = colourTex.createView();
        ...
        this.#lightBuffer = this._device.createBuffer({
            label: "cube light uniforms",
            size: Cube.lightData.byteLength,
            usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST
        });
        this._device.queue.writeBuffer(this.#lightBuffer, 0, Cube.lightData, 0, Cube.lightData.length);
    }
    ...
    #setupPipeline(presentationFormat: GPUTextureFormat, shaderModule: GPUShaderModule) {
        if (!this.#uniformBuffer || !this.#lightBuffer || !this.#cubeTextureSampler || !this.#cubeTextureView)
        ...
        this.#bindGroup = this._device.createBindGroup({
            ...
            entries: [
                ...
                //         offset: Cube.ambientLightOffset,
                //         size: vec4SizeBytes
                //     }
                // },
                {
